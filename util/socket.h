...
public:
	...
	BOOL Bind(USHORT nPort=0, string ip = "")
	{
		...
		{
			int on = 1;
			setsockopt(m_hSock, SOL_SOCKET, TCP_NODELAY, (const char*) &on, sizeof(on));
	...
	BOOL Connect(string ip, USHORT port, LONG lTOSMilisec = -1)
	{
		...
		if( lTOSMilisec > 0 )
		{
			setsockopt(m_hSock, SOL_SOCKET, TCP_NODELAY, (char*) &lTOSMilisec, sizeof(lTOSMilisec));
		}

		int ret = connect(m_hSock, (sockaddr*)&sockAddr, sizeof(sockAddr));
		
		if( ret >= 0 && lTOSMilisec > 0 )
			setsockopt(m_hSock, SOL_SOCKET, TCP_NODELAY, (char*) &dw, sizeof(dw));
		...
		if( lTOSMilisec > 0 )
		{
			...
			setsockopt(m_hSock, SOL_SOCKET, TCP_NODELAY, &tv, sizeof(tv));
		}
